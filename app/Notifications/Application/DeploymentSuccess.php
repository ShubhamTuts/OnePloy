...
class DeploymentSuccess extends CustomEmailNotification
{
    ...
    public function toMail(): MailMessage
    {
        ...
        if ($pull_request_id === 0) {
            $mail->subject("OnePloy: New version is deployed of {$this->application_name}");
        } else {
            $fqdn = $this->preview->fqdn;
            $mail->subject("OnePloy: Pull request #{$pull_request_id} of {$this->application_name} deployed successfully");
        }
    ...
    {
        if ($this->preview) {
            $message = 'OnePloy: New PR'.$this->preview->pull_request_id.' version successfully deployed of '.$this->application_name.'';
            if ($this->preview->fqdn) {
                $buttons[] = [
