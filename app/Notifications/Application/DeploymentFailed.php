...
class DeploymentFailed extends CustomEmailNotification
{
    ...
    public function toMail(): MailMessage
    {
        ...
        if ($pull_request_id === 0) {
            $mail->subject('OnePloy: Deployment failed of '.$this->application_name.'.');
        } else {
            $fqdn = $this->preview->fqdn;
            $mail->subject('OnePloy: Deployment failed of pull request #'.$this->preview->pull_request_id.' of '.$this->application_name.'.');
        }
    ...
    {
        if ($this->preview) {
            $message = 'OnePloy: Pull request #'.$this->preview->pull_request_id.' of '.$this->application_name.' ('.$this->preview->fqdn.') deployment failed: ';
        } else {
            $message = 'OnePloy: Deployment failed of '.$this->application_name.' ('.$this->fqdn.'): ';
        }
        $buttons[] = [
