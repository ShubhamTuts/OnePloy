...
class ContainerRestarted extends CustomEmailNotification
{
    ...
    public function toMail(): MailMessage
    {
        ...
        $mail->subject("OnePloy: A resource ({$this->name}) has been restarted automatically on {$this->server->name}");
    ...
    {
        $message = "OnePloy: A resource ({$this->name}) has been restarted automatically on {$this->server->name}";
        $payload = [
            'message' => $message,
        ];
        if ($this->url) {
