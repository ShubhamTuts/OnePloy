...
class TaskFailed extends CustomEmailNotification
{
    ...
    public function toMail(): MailMessage
    {
        ...
        $mail->subject("OnePloy: [ACTION REQUIRED] Scheduled task ({$this->task->name}) failed.");
    ...
    {
        $message = "OnePloy: Scheduled task ({$this->task->name}) failed with output: {$this->output}";
        if ($this->url) {
            $buttons[] = [
                'text' => 'Open task in OnePloy',
                'url' => (string) $this->url,
            ];
        }
