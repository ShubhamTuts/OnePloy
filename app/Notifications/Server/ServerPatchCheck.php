...
class ServerPatchCheck extends CustomEmailNotification
{
    ...
    public function toMail($notifiable = null): MailMessage
    {
        ...
        if (isset($this->patchData['error'])) {
            $mail->subject("OnePloy: [ERROR] Failed to check patches on {$this->server->name}");
        ...
        $mail->subject("OnePloy: [ACTION REQUIRED] {$totalUpdates} server patches available on {$this->server->name}");
    ...
    public function toDiscord(): DiscordMessage
    {
        ...
        if (isset($this->patchData['error'])) {
            ...
            return new DiscordMessage(
                title: ':x: OnePloy: [ERROR] Failed to check patches on '.$this->server->name,
        ...
        return new DiscordMessage(
            title: ':warning: OnePloy: [ACTION REQUIRED] Server patches available on '.$this->server->name,
            description: $description,
            color: DiscordMessage::errorColor(),
        );
    ...
    public function toTelegram(): array
    {
        ...
        if (isset($this->patchData['error'])) {
            ...
            $message = "❌ OnePloy: [ERROR] Failed to check patches on {$this->server->name}!\n\n";
        ...
        $message = "🔧 OnePloy: [ACTION REQUIRED] {$totalUpdates} server patches available on {$this->server->name}!\n\n";
        $message .= "📊 Summary:\n";
        $message .= '• OS: '.ucfirst($osId)."\n";
        $message .= "• Package Manager: {$packageManager}\n";
    ...
    public function toSlack(): SlackMessage
    {
        ...
        if (isset($this->patchData['error'])) {
            ...
            return new SlackMessage(
                title: 'OnePloy: [ERROR] Server patch check failed',
                description: $description,
        ...
        return new SlackMessage(
            title: 'OnePloy: [ACTION REQUIRED] Server patches available',
            description: $description,
            color: SlackMessage::errorColor()
        );
    }
