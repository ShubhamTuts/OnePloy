        ...
        try {
            ...
            switch ($type) {
                ...
                case 'invoice.payment_failed':
                    ...
                    $subscription = Subscription::where('stripe_customer_id', $customerId)->first();
                    if (! $subscription) {
                        // send_internal_notification('invoice.payment_failed failed but no subscription found in OnePloy for customer: '.$customerId);
                        break;
                    }
                    $team = data_get($subscription, 'team');
                    if (! $team) {
                        // send_internal_notification('invoice.payment_failed failed but no team found in OnePloy for customer: '.$customerId);
                        throw new \RuntimeException("No team found in OnePloy for customer: {$customerId}");
                    }
                ...
                case 'payment_intent.payment_failed':
                    ...
                    $subscription = Subscription::where('stripe_customer_id', $customerId)->first();
                    if (! $subscription) {
                        // send_internal_notification('payment_intent.payment_failed, no subscription found in OnePloy for customer: '.$customerId);
                        break;
                    }
                    if ($subscription->stripe_invoice_paid) {
                ...
                case 'customer.subscription.updated':
                    ...
                    if ($status === 'unpaid') {
                        ...
                        if ($team) {
                            ...
                            // send_internal_notification('Subscription unpaid but no team found in OnePloy for customer: '.$customerId);
                            throw new \RuntimeException("No team found in OnePloy for customer: {$customerId}");
                        }
                    }
                    if ($status === 'active') {
                ...
                case 'customer.subscription.deleted':
                    ...
                    if ($subscription) {
                        ...
                        if ($team) {
                            ...
                            // send_internal_notification('Subscription deleted but no team found in OnePloy for customer: '.$customerId);
                            throw new \RuntimeException("No team found in OnePloy for customer: {$customerId}");
                        }
                    } else {
                        // send_internal_notification('Subscription deleted but no subscription found in OnePloy for customer: '.$customerId);
                        break;
                    }
                    break;
                default:
